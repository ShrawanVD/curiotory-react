...
function Meettutor() {
  ...
  const fetchTeachers = () => {
    ...
    fetch(`${config.apiUrl}/teachers?page=${currentPage}&limit=${itemsPerPage}`)
      .then((response) => {
      ...
      .then((data) => {
        if (!data || !Array.isArray(data.teacher))
          throw new Error("Invalid data format");
        setTeachers(data.teacher);
        setTotalPages(data.totalPages); // Assuming your backend sends total pages
  ...
  return (
    <div>
      ...
      <div className={Meetcss.containerFluid}>
        ...
        {/* <div className={Meetcss.pagination}>
          <button
